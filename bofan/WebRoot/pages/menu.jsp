...
<html>
  <head>
    ...
    <script type="text/javascript">
    	$(function(){
    		$("#treegrid").treegrid({
    			url:"<%=path%>/menuAction/queryTreegrid.action",
    			fit:true,
    			idField:'id',    
    			treeField:'name',    
    			columns:[[   
							...
							if(!row.children) {
								$.messager.confirm("提示","确定删除？",function(rr){
									if(rr) {
										showLoading();
										$.ajax({
	    									url:"<%=path%>/menuAction/deleteMenu.action",
    										type:"POST",
    										data:{id:row.id},
    										dataType:"json",
    										success:function(r){
    			...
    			buttons:[{
    				...
    				handler:function(){
    					if($("#form1").form("validate")){
    						...
    						data = addStrToBeforeKey(data,"menu.");
    						showLoading();
    						$.ajax({
    							url:"<%=path%>/menuAction/saveOrUpdateMenu.action",
    							type:"POST",
    							data:data,
    							dataType:"json",
    							success:function(r){
